...
class TestConvert:
    ...
    def test_conversion_factor(self) -> None:
        factor = Q_(1, "mg/L").to("g/L").magnitude
...
class TestCheckCompatibility:
    def test_psi_compatible_with_kpa(self) -> None:
        assert Q_(1, "psi").is_compatible_with(Q_(1, "kPa"))

    def test_psi_incompatible_with_gallon(self) -> None:
        assert not Q_(1, "psi").is_compatible_with(Q_(1, "gallon"))

    def test_same_unit_compatible(self) -> None:
        assert Q_(1, "meter").is_compatible_with(Q_(1, "meter"))

    def test_length_units_compatible(self) -> None:
        assert Q_(1, "foot").is_compatible_with(Q_(1, "kilometer"))
...
class TestListCompatibleUnits:
    def test_psi_includes_pascal(self) -> None:
        units = get_all_compatible_units(Q_(1, "psi").dimensionality)
        assert "pascal" in units

    def test_length_includes_imperial(self) -> None:
        units = get_all_compatible_units(Q_(1, "meter").dimensionality)
